...
function QuizzesPageContent() {
  ...
  // Local state so we can update it immediately on submit without waiting for refetch
  const [completedQuizId, setCompletedQuizId] = useState<Set<number>>(new Set());

  // Keep in sync whenever historyList loads or refetches
  useEffect(() => {
    if (!historyList) return;
    setCompletedQuizId(new Set(historyList.map((h) => h.quiz_id)));
  }, [historyList]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // so the list shows "Selesai" the moment the user goes back,
      // without waiting for the background refetch to finish
      setCompletedQuizId((prev) => new Set(prev).add(activeQuiz.id));
  ...
  const handleReset = () => {
    ...
    setSubmitError(null);
    setStartError(null);
    // Refetch in background to fully sync history and quiz list
    refetchHistory();
    refetchQuizzes();
  };
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto h-full flex flex-col justify-center min-h-[80vh]">
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg text-xs font-bold transition-all border border-rose-100 shrink-0"
          >
      ...
      {isChecked && (
        <div className="mt-4 flex items-center gap-3 bg-green-50 border border-green-200 text-green-700 px-5 py-3 rounded-xl">
          ...
          <p className="text-sm font-semibold">
            Jawaban dikunci.{" "}
            {isLast ? 'Tekan "Selesai" untuk mengumpulkan.' : "Lanjut ke soal berikutnya."}
          </p>
        </div>
      )}
        ...
        {isLast ? (
          !isChecked ? (
            ...
            <button
              ...
              className="flex items-center gap-2 px-8 py-3 bg-green-600 hover:bg-green-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold rounded-xl shadow-md shadow-green-200 hover:shadow-lg transition-all active:scale-[0.98]"
            >
              {isSubmitting ? (
                <><Loader2 className="w-5 h-5 animate-spin" /><span>Mengirim...</span></>
              ) : (
                <><CheckCircle2 className="w-5 h-5" /><span>Selesai &amp; Lihat Hasil</span></>
              )}
            </button>
          )
        ) : (
          !isChecked ? (
            ...
            <button
              onClick={() => setCurrentIndex((i) => i + 1)}
              className="flex items-center gap-2 px-8 py-3 rounded-xl font-semibold bg-white border border-slate-200 shadow-sm text-indigo-600 hover:bg-indigo-50 hover:border-indigo-200 active:scale-95 transition-all"
            >
              Selanjutnya
      ...
      {!canProceed && !isChecked && (
        <p className="text-center text-xs text-slate-400 mt-3">
          Jawab soal ini terlebih dahulu untuk melanjutkan.
        </p>
      )}
    </div>
  );
